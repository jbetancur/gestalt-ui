    ...
    describe('when the workflow is invoked', () => {
      ...
      it('should return a payload and dispatch a success status and merge any env vars', () => {
        result = saga.next([
          ...
          {
            data: [
              { name: 'action A', properties: { ui_locations: ['streamspec.edit'] } },
              { name: 'action B', properties: { ui_locations: ['streamspec.instances'] } },
            ]
          },
        ]);
        ...
        const payload = {
          ...
          datafeeds: [{ name: 'a feed' }],
          lambdas: [{ name: 'a lambda', properties: { runtime: 'java' } }],
          actions: [{ name: 'action A', properties: { ui_locations: ['streamspec.edit'] } }],
          instanceActions: [{ name: 'action B', properties: { ui_locations: ['streamspec.instances'] } }],
        };

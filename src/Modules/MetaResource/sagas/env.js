...
export function* fetchEnvSchema(action) {
  try {
    ...
    yield put({ type: types.FETCH_ENVSCHEMA_FULFILLED, payload });
  } catch (e) {
    yield put({ type: types.FETCH_ENVSCHEMA_REJECTED, payload: e.message });
  }
}
...
export default function* () {
  ...
  yield fork(takeLatest, types.FETCH_ENVSCHEMA_REQUEST, fetchEnvSchema);
}

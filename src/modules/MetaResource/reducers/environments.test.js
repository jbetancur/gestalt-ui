...
describe('environments reducer', () => {
  ...
  it('should return the initial state on UNLOAD_ENVIRONMENTS', () => {
    expect(
      reducer({ environments: [{ id: 1 }] }, metaActions.unloadEnvironments())
    ).to.deep.equal(initialState);
  });

  it('should handle LOCATION_CHANGE', () => {
    expect(
      reducer({ environments: [{ id: 1 }] }, { type: LOCATION_CHANGE })
    ).to.deep.equal(initialState);
  });
